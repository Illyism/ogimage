...
# Next still resolves @types/react during `next build` even when
# ignoreBuildErrors is on. If that package is missing, Next tries to
# `npm install` it. oven/bun images have no npm. CI=1 makes Next fail

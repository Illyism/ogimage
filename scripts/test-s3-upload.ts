...
async function testUpload() {
  const testImagePath = join(process.cwd(), 'a039d3d3-8a42-450c-bd4c-0d86d70f497a.jpeg')
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`✅ Downloaded: ${downloadedBlob.size} bytes, type: ${downloadedBlob.type}`)
      ...
      if (downloadedSignature[0] === 0xff && downloadedSignature[1] === 0xd8 && downloadedSignature[2] === 0xff) {
